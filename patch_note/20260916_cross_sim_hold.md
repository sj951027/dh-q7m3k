...
## 같은 날 추가 (21:4x, 배치 후)
- 09-16 배치: 컨센서스 실패 감지가 첫 작동(커버리지 0/2518 → 저장 안 함·종료 1) → 알림 첫 줄이 "⚠️ 확인 필요"로 나감. 스크리너 결과와 무관한 관측 수집기 실패라 오해 소지.
- `notify_telegram.py`: 실패 단계를 관측 전용(fetch_consensus·dart_events·dart_backfill)과 그 외로 나눠, 관측 전용 실패는 🔔 줄("관측 수집 실패(점수·추천 무관)")로만 알리고 첫 줄은 ✅ 유지. 미발송 프리뷰 확인.
