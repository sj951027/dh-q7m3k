# 2026-09-11 — 네이버 수급 크롤링 소멸 · 수급 소스 KIS daily_flows 교체 (2026.09.4 · 판정 규약 무변경, 점수는 소스 교체분만 변화)

> ⚠ 아래 본문의 '0-diff'·'수급 0이 유니버스를 줄였다' 두 표현은 **9/12 추가 절에서 정정**했다. 정정본이 정본.
...
## 영향범위
- **점수식·가중·임계값·판정·게이트 무변경.** 입력 공급처만 교체. 겹치는 run의 '커버되는 종목'에선 수급 성분이 동일했으나, 코스닥 글로벌·신규코드는 소스에서 통째로 빠져 있었다(9/12 절에서 수정).
...
## 추가 (2026-09-12 새벽 재실행 + 코스닥 글로벌 누락 수정)

### 09-11 재실행 (수급 0 → KIS 수급)
`research/clean_partial_run.py 20260911 --yes --force` 후 `run_manual_logged.bat` 재실행(23:54~01:42, 단일 실행 = 앵커 보존).
적재 실측: stage1 2,357 · stage3 540(코스피 175·코스닥 365) · v3_scores 540 · lowvol 1,036 · wu 4,272 · large 500/500.
동결 시각 단일(00:59:50) — 이중실행 게이트 해당 없음. 완전성 점검 통과, 텔레그램·push 정상.

이전(수급 0) run 대비 실측: v30 점수 동일 38.8% · 평균 −3.22 · 순위상관 0.966 · 시장별 상위 10 겹침 4/10(코스피)·3/10(코스닥).
lv_b 는 점수 동일 43.2%이나 **상위 10 겹침 10/10**(양 시장) — 수급은 lv_b 점수식에 없고 유니버스 경로로만 들어간다.
유니버스 크기는 수급 복구와 거의 무관했다: stage3 540(이전 539)·lv_b 184(이전 183). 9/11 유니버스 축소(전일 608→540)는 장세 영향이고,
앞 절에서 "수급 0의 몫이 있다"고 쓴 추정은 **틀렸다**(실측으로 기각).

### 코스닥 글로벌·신규코드 누락 (앞 절 "0-diff" 표현 정정)
- 증상: 재실행 로그에서 수급 결손 코스피 5/265·코스닥 40/714, 코스닥 레짐 외인 proxy 3/10 → 외인 점수 0.
- 원인: `universe_ohlcv.py`가 FDR `Market`이 정확히 `KOSPI`/`KOSDAQ`인 **숫자 6자리**만 적재 → `KOSDAQ GLOBAL` 50종목(알테오젠·에코프로·에코프로비엠 등)과
  영문 섞인 신규코드 81종목이 ohlcv에 없다. KIS 수급 유니버스가 ohlcv를 쓰므로 그대로 전파됐다(2026-06-29 이후 결손).
- 앞 절의 "09-08·09-09 stage3 100% 동일"은 그날 stage3에 이 종목들이 없었기 때문이며, **전체가 0-diff라는 뜻은 아니다**. 표현을 정정한다.
- 수정: `kis_flows.py` `load_all_tickers(include_extras=True)` — ohlcv 유니버스에 `listing_cache`(상장목록) 종목을 보충(245종목, 네트워크 0).
  보충은 **수급 단계에만** 적용하고 공매도 단계(`--no-daily` 호출)는 종전 유니버스 그대로 → sv_a 입력 불변. ohlcv(시세) 유니버스는 건드리지 않는다
  (lowvol/wu 모델 유니버스가 바뀌므로 별건 — 미결 과제로 남김).
- 09-11 백필 실측: 결손 208종목 재시도 전부 성공 → 09-11 수급 2,520 → 2,765종목. 앞선 일괄 실행에서 일부가 조용히 실패했던 것(재시도로 복구, 자가치유 설계대로).
- 보충 후 재계산(오프라인): 코스닥 레짐 외인 **−2**(외인이탈_강함, 10/10 합산, 현재 적재값은 0) · stage3 결손 18종목 수급점수 대부분 −10.
  즉 **현재 09-11 적재본에는 코스닥 레짐 −2와 18종목 수급이 빠져 있다**(3차 재실행 여부는 사용자 결정).
