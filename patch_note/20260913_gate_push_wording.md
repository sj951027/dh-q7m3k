...
**v3(같은 날 재검토)**: v2 의 국면 정의('전종목 중앙값 40일 수익 ±2%')는 중앙값 종목이 평소에도 마이너스라(3년 실측 코스피 −1.3%·코스닥 −3.1%) 하락 44~59%로 쏠림 →
**지수(코스피·코스닥 평균) 40일 수익 ±3%** 로 교체(3년 분포 하락 27·횡보 30·상승 43%). 주블록 8 = "겹치지 않는 창 3개"라는 v2 의 환산이 틀려서 정정(실제 ≈ 2개).
대상 모델을 lv_b·v30 으로 명시(sv_a 는 §11 유의 전 제외). 크기 문턱을 40일 초과 **+1.35%p** 로 환산 명기. 용량 조항은 판정 계산 밖 '자기 점검'으로 분리.
2026-06 이후 그림자 앵커는 이 정의로 88% '하락' — 두 번째 국면은 시장이 줘야 함.
**v4(외부 검토 반영)**: chatgpt.com(Plus, High) 에 v3 전문을 붙여 "허점 찾기" 10건 수령 — `research/GPT_REVIEW_ops_adoption_v3_20260913.md` 에 원문·판단 기록.
9건 타당·1건 절반. 반영: 주 잣대 중앙값→**PIT 동일가중 평균**(중앙값 음수 편향) · 판정 CI **40일 블록 부트스트랩** 단일(iid 제거) · 최소 표본 **비중첩 코호트 4** ·
국면당 코호트 ≥2 + 국면별 평균>0 · **관찰기간 고정 2026-09-15~2027-03-31, 판정 1회, 미충족=판정불가**(선택적 중단 차단) · 처음부터 Bonferroni α/2 ·
라벨 우선순위 논리식 · 체결 규칙(정지·결측=현금) · 수익 정의(수정주가·배당 제외) · 크기 산식 E ≥ c+1.0 · 용량 = 게이트 아님 · 등록 시 commit hash 동결.
첫 판정 예정 **2027-06 초**. 미등록. 협업 방식: API 없이 브라우저 조작(로그인은 사용자 세션), 토큰 과금 없음.
