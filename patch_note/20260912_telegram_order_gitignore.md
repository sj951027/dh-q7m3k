# 2026-09-12 — 텔레그램(순서·문안) · DART 재무 백필 · 저장소/백업 정리 (2026.09.5 · 판정·점수 0-diff)
...
## 추가 — 텔레그램 문안 v3 (lv_b 우선 · 섹션형 · 데이터 날짜)

### 왜
사용자 실거래는 **lv_b**다(OPS_GUIDE §0 · PTW `#저변동`). 그런데 알림은 v30 을 앞세우고,
매일 누르는 '종목 상세' 링크도 v30 기준 `filter.html` 이었다. 화면과 실제 운용이 어긋나 있었다.
그 밖에 캡처로 확인된 읽기 문제: 한 줄에 항목을 3~5개 이어 붙여 폰에서 줄이 접힘 · 판정 줄에 모델 7개 나열 ·
'달라진 것: 없음'이 매일 표시 · 링크 3줄.

**날짜 결함**: 제목이 `datetime.now()` 라서 9/11 장 데이터를 9/12 로 표기했다(자정 넘김·주말 재실행 시 어긋남).

### 무엇을
| 항목 | 전 | 후 |
|---|---|---|
| 제목 날짜 | 발송 시각 | **stage3_final 최신 run_id**(`9/11(금)`) |
| 모델 순서 | v30 → lv_b | **lv_b(운용) → v30(참고)**, 판정 라벨은 그대로 병기 |
| 돈 | 한 줄에 3개 나열 | `<pre>` 3줄 정렬 + `← 운용 중`·`← 판정 임박` 꼬리표 |
| 판정 | 7개 한 줄(D-4 식) | 남은 일수로 묶어 **최대 3줄**, 나머지는 리더보드 |
| 달라진 것 | 매일 '없음' 출력 | **있을 때만** 표시 |
| 링크 | 4개(필터·대형·저변동·리더보드) | **2개**(저변동 종목 보기 · 리더보드) — 저변동/대형은 리더보드에서 모델명 클릭으로 이동 |

돈 줄에 7개 전부를 싣지 않은 이유: 한 달 수익 줄 세우기가 되면 트랙 간 비교 금지 원칙과 어긋난다.
운용 2개 + 판정 임박 1개만 싣고 나머지는 리더보드에 둔다.

### 어떻게
- `notify_telegram.py`: `_status_lines_v3()` · `_change_events_v3()` · `_date_head_v3()` 신설, `build_message()` 교체.
  **종전 v2(`_model_status_lines_v2`)와 링크 상수는 그대로 보존** — 되돌리려면 `build_message()` 의 lines 구성만 v2 로 바꾸면 된다.
- 운용 모델을 코드에 박지 않는다: `docs/models_registry.json` 에 `"live": "lv_b"` · `"reference": "v30"` 추가,
  `_apply_registry()` 가 읽는다. 나중에 운용 모델이 바뀌면 원장 한 줄만 고치면 알림 순서가 따라온다.
- 배포 보류(게이트 실패) 알림 경로는 무변경.

### 검증 (발송 없음)
- `build_message()` 렌더: 18줄·506자, HTML 태그 균형 정상(파서 검사).
- 예외 경로: `cross_sim.json` 없음 → 돈 섹션만 빠지고 12줄 정상 · `leaderboard.json` 없음 → 안내 한 줄로 6줄 정상.
- `_date_head_v3()` = `9/11(금)`(run_id 20260911) — 발송 시각 9/12 와 다른 것이 정상.
- `python tests/run_tests.py` 통과.
