...
# ====================================================================================
# 정본 판정(VERDICT 봉인) — 판정·은퇴 시 갱신(docs/leaderboard_full.html SEALED 맵과 동일하게 유지 — 2026-09-18 첫 화면 교체 후 인라인 맵은 _full 쪽)
SEALED_V2 = {"v30": "유의(8/09 정본)", "lv_b": "기움(8/29 정본)",
             "lv_a": "노이즈(8/29)", "mom_a": "노이즈(8/29)", "sm_a": "노이즈(9/01)",
             "sv_a": "기움(9/13 정본)", "le_a": "노이즈(9/13)", "mom_b": "역작동·은퇴(9/17 정본)", "qs_a": "노이즈(9/17 정본)"}
    ...
    try:
        ...
        if bf.exists():
            ...
            if names:
                ...
                if crit:
                    ev.append(f"⚠️ 배치 단계 실패: {' '.join(crit)} — 로그 확인")
                if obs:
                    ev.append(f"관측 수집 실패(점수·추천 무관): {' '.join(obs)} — 다음 실행 때 재시도")
    except Exception:
        pass
    return ev
