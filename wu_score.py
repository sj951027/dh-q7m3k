...
FACTORS = {
    ...
    # [2026-08-29 추가 — PREREGISTER_sv_b.md 초안] 신용잔고비율 5일 평균(높을수록 좋음 가설).
    #   svr5 와 동일한 배치순서 주의(1일 지연 허용)를 스펙에 포함. 보조 전용(NaN=0.5).
    "crb5":   (True,  "mean(short_flows.credit_bal_rate,5,min_periods=1)"),
...
MODELS = {
    ...
    # [사전등록 초안 — PREREGISTER_sv_b.md, 적용은 sv_a §11 판정 후] sv_a + 신용잔고비율.
    #   근거: research/RESEARCH_candidates_scan_20260829.md §4 — sv_a 짝비교 h10/h20 CI>0,
    #   축 직교(sv_a점수↔신용 -0.06). 변수 1개 원칙. 신규 모델은 소급 적재 없음(다음 run부터).
    "sv_b": ["svr5", "crb5"],
...
def compute_frames(W):
    ...
    F["crb5"] = W["crb"].rolling(5, min_periods=1).mean()   # [sv_b 초안] 보조 전용(적재 3일 지연 허용)
