...
def main():
    ...
    for i, row in df.iterrows():
        annual = get_annual_metrics(row['corp_code'], DART_API_KEY)
        quarterly = get_quarterly_yoy(row['corp_code'], DART_API_KEY)

        pattern, fund_score = classify_pattern(annual['annual_yoy_%'], quarterly['quarterly_yoy_%'])

        # [V2.6 패치] sector를 받아서 산업별 OCF 임계값 적용
        ...
        if annual.get('annual_yoy_%') is not None:
            annual_op_count += 1
        if quarterly.get('quarterly_yoy_%') is not None:
            quarterly_op_count += 1
        if annual.get('ocf_latest_억') is not None:
            ocf_found_count += 1
        ...
        results.append({
            ...
            **quarterly,
            ...
            'ocf_threshold_group': ocf_group,    # lenient / default / strict
        })

        if (i + 1) % 20 == 0 or (i + 1) == len(df):
            name = str(row.get('name', row['ticker']))[:10]
            basis = quarterly.get('q_basis') or '-'
            print(f"   [{i+1:>3}/{len(df)}] {name:<12} "
                  f"→ {pattern} / Q:{basis} / OCF:{ocf_pattern}({ocf_group[:3]})")
