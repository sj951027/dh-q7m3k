...
- lead 트랙(2026-09-24~): 월 1회 픽 동결(`lead_picks`) · 잣대 = 120일 보유 수익의 픽비중 지수/동일가중/대조군(거래대금) 3중 · 비겹침 3창 후 판정(PREREGISTER_ld_a §3). h20 IC·리더보드 미적용.
...
| ops | **운용 채택(lv_b·v30)** | 돈 잣대 사전등록 v5 — 시장별 상위10·동일가중·희석 제외·t+1 종가·40일 보유 · 초과 = 같은 시장 전종목 동일가중 평균 대비 · 40일 블록 CI 97.5%(α/2) · 코호트≥4·국면≥2 · 비용 0.35 정본 · 점프컷 미적용. **§11 판정을 대체하지 않음(두 라벨 병기)** | **20260916**(관찰 진입 20260917~20270331) | **2027-06 초 1회** | PREREGISTER_ops_adoption.md |
| **lead** | **ld_a** | 고베타×최근 신고가 월간 top20·120일 보유·그룹당 4 상한 — **등록 2026-09-24**(새 트랙, §11 미적용·분모 분리, spec_hash 4f01c8fcfb95, 대조군 ld_ctl_amt). 가중 0·표시 docs/lead.html(성적표 표 미포함) | **20261001**(첫 앵커) | 비겹침 120일 창 3개 → **≈2028-03** | PREREGISTER_ld_a.md |
...
## 4차 판정 (2026-09-13 — VERDICT_20260913_wu_sv_le.md **정본 확정**)
| 트랙 | 모델 | h20 IC | 95% CI | 판정 |
|---|---|---|---|---|
| wu | sv_a | +0.0330 | [+0.0148, +0.0505] | **기움**(채택 아님) — 형식 기준(CI>0·주별 80%·Bonferroni 11/6) 충족이나 주블록 하단 +0.0017(경계)·상승장 앵커 7개 IC≈0(관전 포인트 ② 미확인) |
| wu | le_a | −0.0145 | [−0.0436, +0.0152] | 노이즈 — 주별 20%, h5 우위도 미재현 |

- sv_a−le_a 짝비교 +0.0476 [+0.017, +0.078] (sv_a 우위 유의). 앵커 20개(7/15~8/12) 단일 국면.
- 처분: 둘 다 **적재 유지·가중 0**. sv_a = wu 트랙 표시 기준(sv.html '기움' 라벨). sv_a 재판정 조건 = 상승장(KOSDAQ>SMA20) 앵커 ≥15 또는 OOS 80 중 이른 때.
- sv_b **등록(9/13, REG_DATE 20260914)** — 사용자 결정으로 qs_a 판정 직전 등록을 감수(분모 7). qs_a 판정문에 분모 7 명기.
