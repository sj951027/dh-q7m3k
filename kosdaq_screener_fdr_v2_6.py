...
V2.4 → V2.6 신규 추가:
  ...
  ⑭ 외국인 KOSDAQ 누적 순매수 (시총 상위 10개 종목 합산)
  ...
  최종 regime_score = KOSDAQ 점수 + 환율 점수 + 외인 점수
  → 약세장 + 원화약세 + 외인이탈 동시면 최대 -16점까지 페널티

V2.4 기존 기능 유지:
  ⑦ 시장 레짐 필터 (KOSDAQ 지수 분석)
...
[출력]
    v2_kosdaq_oversold_*.csv  (2/3단계와 호환)
...
# [V2.6 NEW] 외국인 KOSDAQ 누적용 시총 상위 대형주
# 시총 상위 10개가 KOSDAQ 시총의 약 50%+ 차지. 외인은 거의 대형주만 매매.
...
# [코스닥 추가] 스팩(SPAC) — 합병 전 가격 변동은 의미 없음, 매우 많음
...
# ============================================================
# [V2.4] 시장 레짐: KOSDAQ 부분
# [V2.6] + 환율(USD/KRW) + 외국인 KOSDAQ 누적 통합
# ============================================================

def _analyze_kosdaq_regime():
    """
    KOSDAQ 종합지수(KS11) 분석으로 시장 레짐 판단.
    ...
    try:
        df = fdr.DataReader('KQ11', from_date)
        if df is None or len(df) < 200:
            print("   ⚠️  KOSDAQ 지수 데이터 부족 (200일 이상 필요)")
            return None
        ...
        print(f"   ⚠️  KOSDAQ 지수 조회 실패: {str(e)[:80]}")
        return None
...
def _analyze_fx_trend():
    ...
    환율 상승(원화 약세) → 외국인 환차손 회피 매도 → KOSDAQ 매도 압력.
    KOSDAQ 지수만 보는 기존 레짐은 이 신호를 놓침.
    """
    today = datetime.now()
...
def analyze_market_regime():
    """
    [V2.6] 통합 시장 레짐 분석.
    KOSDAQ 추세 + 환율 + 외국인 KOSDAQ 누적을 모두 합산해서 통합 regime_score 산출.

    KOSDAQ 데이터가 없으면 None 반환 (FX/Flow만 있어도 의미 약함).
    FX/Flow는 sub-실패해도 점수 0으로 처리하고 계속 진행.
    """
    kospi = _analyze_kosdaq_regime()
    if not kospi:
        return None
...
def print_regime_box(regime_info):
    """시장 레짐 정보 박스 출력 (KOSDAQ + 환율 + 외인 통합)"""
    if not regime_info:
        print(f"\n{'─'*72}")
    ...
    print(f"{'═'*72}")
    print(f"   KOSDAQ: {r.get('kospi', 0):,.2f}  ({daily_sign}{daily:.2f}% vs 어제)  "
          f"→ KOSDAQ 점수: {r['regime_kospi_score']:+d}")
...
def run_screener():
    print(f"\n{'='*72}")
    print(f"📊  [V2.6] KOSDAQ 과매도 스크리너 (레짐 + 환율 + 외인 통합)")
    print(f"     • 금융주/리츠 자동 제외")
    print(f"     • 매집 / 거래량 / 추세 전환")
    print(f"     • 외국인/기관 수급 (네이버)")
    print(f"     • 시장 레짐 + 환율(USD/KRW) + 외국인 KOSDAQ 누적  ← V2.6")
    print(f"{'='*72}")
    print(f"실행 시각: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

    # ===== [V2.6] 통합 레짐 분석 (KOSDAQ + 환율 + 외인) =====
    print("0️⃣  시장 레짐 분석 (KOSDAQ + USD/KRW + 외국인 KOSDAQ)")
    regime_info = analyze_market_regime()
    print_regime_box(regime_info)

    print("1️⃣  종목 유니버스 구성")
    tickers, name_map, sector_map = get_kosdaq_universe()
    if not tickers:
    ...
    # ===== [V2.6] 통합 레짐 점수 + 메타 컬럼 부여 (KOSDAQ + 환율 + 외인) =====
    if regime_info:
    ...
    if regime_info:
        ...
        print(f"      └ KOSDAQ {regime_info['regime_kospi_score']:+d} / "
    ...
    if regime_info and regime_info['regime_score'] <= -8:
        ...
        if regime_info['regime_kospi_score'] < 0:
            details.append(f"KOSDAQ 약세({regime_info['regime_kospi_score']:+d})")
    ...
    filename = f"v2_kosdaq_oversold_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
